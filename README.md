...
[下载 Mac / Windows 插件运行包](downloads/LightCut-Mac-Windows-Preview-R81.zip) · 版本 **0.4.1-preview.1**

包含插件、运行源码、独立依赖安装器和检查工具；不包含剪映应用、上游原生引擎或官方资源。**当前适配剪映专业版11.5.0，Windows已验收构建11.5.0.14471。** 首次使用需要已有合法、版本匹配的原生引擎；包内安装器不会替用户下载未获分发授权的引擎。已有R8用户可复用引擎来源记录升级。

将ZIP解压后交给Codex，先读取包内 `START-HERE.md` 和 `install/README.md`，由Agent准备环境。不要直接复制覆盖旧程序目录。新电脑缺少引擎时会明确停止原生草稿构建，不属于完整零配置安装。

校验 SHA-256：`1c73764de728df02b110aad5d115725afbad6ad768c59ef0b37ee90499da457e`。
...
> **当前是早期开发阶段。** 本仓库提供原创项目介绍、静态展示页和开发文档，现已提供插件运行源码与预览安装包，但未附带上游原生引擎。下表所列实现来自本地研究版本，不能通过克隆这个公开仓库直接运行。Windows 指定环境已完成草稿和字幕接缝样片验收；全新电脑安装仍有引擎来源缺口。
...
- **想了解或试用**：先看 [使用与试用说明](docs/getting-started.md)，以及 [平台兼容性](docs/platforms.md)。现有公开预览包，首次使用须具备匹配引擎。
