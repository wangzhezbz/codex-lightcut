...
for p in sorted(ROOT.rglob('*')):
    ...
    count += 1
    if (p.suffix.lower() in FORBIDDEN and p.relative_to(ROOT).as_posix() not in {'downloads/LightCut-Mac-Windows-Preview-R81.zip', 'downloads/LightCut-Mac-Windows-R82.zip', 'downloads/LightCut-Mac-Windows-R84.zip', 'downloads/LightCut-Mac-Windows-R86.zip'}) or parts[0] in {'upstream', 'work', 'local', 'research', 'results', 'dist'}:
